...
import {
  exportAmbulanceSummary as exportAmbulanceSummaryApi
...
function TotalAmbulanceSummaryWrap({ intl, setNhmDashboardView }) {
  ...
  function handleExport(query, payload) {
    setLoader(true);
    exportAmbulanceSummaryApi(query, userConfig.userConfig, payload)
      .then((res) => {
        const fileType = query.fileType;
        const data = res.body;
        const fileName = 'TotalAmbulanceSummary';
        const metadata = {
  ...
  return (
    <div className="TotalAmbulanceSummaryWrapper CommonHeader">
...
TotalAmbulanceSummaryWrap.defaultProps = {
  setNhmDashboardView: () => {},
};

TotalAmbulanceSummaryWrap.propTypes = {
  intl: PropTypes.object.isRequired,
  setNhmDashboardView: PropTypes.func,
};

export default injectIntl(TotalAmbulanceSummaryWrap);

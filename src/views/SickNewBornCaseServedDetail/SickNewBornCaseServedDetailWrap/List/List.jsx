...
import {
  fetchSickNewBornCaseServedDetail as fetchSickNewBornCaseServedDetailApi,
...
function List({ intl, exportAction }) {
  ...
  const {
    data: sickNewBornData,
    request: sickNewBornDataRequest,
    loading: sickNewBornDataLoading,
  } = useService({
    method: fetchSickNewBornCaseServedDetailApi,
    context: userConfig.userConfig,
    initialValues: {},
  });

  const listDetails = sickNewBornData?.data || {};
  ...
  function handleExportAction(fileType) {
    exportAction({
      caseServedCategory: "SICK_NEW_BORN_BABY",
      fileType,
      pageNo: listState.pageNo,
  ...
  useEffect(() => {
    ...
    sickNewBornDataRequest(query);
  }, [listState]);

  return (
    <div className="ListDashboardMaster">
      <div style={{ overflowX: 'auto' }}>
        <ContentWrap isFetching={sickNewBornDataLoading}>
          ...
          <div className="d-flex" style={{ flexDirection: 'row-reverse', alignItems: 'center' }}>
            <span className="d-flex Flex-Direction-Row">
              <Button
                ...
                disabled={listDetails.first || sickNewBornDataLoading}
              ...
              <Button
                className="Border-None"
                disabled={listDetails.last || sickNewBornDataLoading}
                onClick={pageForward}
                style={{
                  backgroundColor: 'transparent',
                }}

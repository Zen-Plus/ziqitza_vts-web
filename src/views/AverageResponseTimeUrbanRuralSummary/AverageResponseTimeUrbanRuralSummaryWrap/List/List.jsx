...
import {
  fetchAverageResponseTimeUrbanRuralSummary as fetchAverageResponseTimeUrbanRuralSummaryApi,
...
function List({ intl, exportAction }) {
  ...
  const {
    ...
    request: avgResponseTimeRequest,
    loading: avgResponseTimeDataLoading,
  } = useService({
    method: fetchAverageResponseTimeUrbanRuralSummaryApi,
    context: userConfig.userConfig,
    initialValues: {},
  });

  const listDetails = avgResponseTimeData?.data || {};
  ...
  useEffect(() => {
    ...
    avgResponseTimeRequest(query);
  }, [listState]);

  return (
    <div className="ListDashboardMaster">
      <div style={{ overflowX: 'auto' }}>
        <ContentWrap isFetching={avgResponseTimeDataLoading}>
          <div className="d-flex Flex-Direction-Column" style={{
            marginBottom: '1.5rem',
            padding: '0 6px',
          }}>
            <div className='d-flex Flex-Space-Between' style={{ alignItems: 'baseline' }}>
              <FilterDate
          ...
          <div className="d-flex" style={{ flexDirection: 'row-reverse', alignItems: 'center' }}>
            <span className="d-flex Flex-Direction-Row">
              <Button
                ...
                disabled={listDetails.first || avgResponseTimeDataLoading}
              ...
              <Button
                className="Border-None"
                disabled={listDetails.last || avgResponseTimeDataLoading}
                onClick={pageForward}
                style={{
                  backgroundColor: 'transparent',
                }}
